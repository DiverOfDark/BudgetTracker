				...
				<div class="card">
					<div class="card-header">
						...
						<div class="card-options">
							...
							<button class="btn btn-outline-primary btn-sm ml-2" on:click="{ () => showControls = !showControls }">
								{#if showControls}
									Скрыть кнопки
								{:else}
									Показать кнопки
							...
							<button class="btn btn-outline-primary btn-sm ml-2" on:click="{ () => exemptTransfers = !exemptTransfers }">
								{#if exemptTransfers}
									Показать переводы
								{:else}
									Скрыть переводы
							...
							{#each vm.values as item, rowIdx}
								<tr>
									...
									{#each item.cells as cell, idx}
										<td class="{cell.isOk ? '' : 'table-danger'}">
