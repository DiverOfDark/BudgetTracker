...
@{
    ...
    <div class="card-body" style="padding: 0; height: 100%;">
        ...
        <svg id="burst-@chartId"></svg>
    </div>
    <script type="text/javascript">
        (function() {
            function scale(value) {
                return Math.max(3, Math.log(Math.abs(value))/2);
            }

            var chartId = 'burst-@chartId';
            var boundingClientRect = $("#" + chartId).parent()[0].getBoundingClientRect();
            $("#" + chartId).attr("width", boundingClientRect.width);
            $("#" + chartId).attr("height", boundingClientRect.height);
            ...
            var simulation = d3.forceSimulation()
                .force("link", d3.forceLink().id(function(d) { return d.id; }).strength(function(d) {
                    return 5/scale(d.source.node.amount);
                }))
                .force("charge", d3.forceManyBody().strength(-500).distanceMax(2000).distanceMin(1))
            ...
            var link = g.append("g")
                ...
                .data(linksArr)
                .enter().append("line")
                .attr("stroke-width", function(d) { return scale(d.target.node.amount); })
            ...
            var nodeObj = g.append("g")
                ...
                .data(nodeArr)
                .enter().append("circle")
                .attr("r", function(d) { return 2*scale(d.node.amount); })
            ...
            var textElements2  = g.append('g')
                ...
                .attr('dy', 15)
                .attr('fill', function(d) { return d.color; })
                .text(function(d) { return d.node.amountFormatted; });
